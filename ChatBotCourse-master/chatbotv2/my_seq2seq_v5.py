...
def load_word_set():
    #file_object = open('./YMC Python3/yingBurgerMovieTitleGenerator/S2sTrainingInput.txt', 'r', encoding="utf-8")
    file_object = open('./S2sTrainingInput.txt', 'r', encoding="utf-8")
    while True:
        line = file_object.readline()
        if line:
            line_pair = line.split('|')
            line_question = line_pair[0].strip('\n')
            line_answer = line_pair[1].strip('\n')
...
def load_vectors(input):
    ...
    words = int(words_and_size.decode().split(' ')[0])
    #words = 22122
    size = int(words_and_size.decode().split(' ')[1])
    #size = 200
    print("words =", words)
    print("size =", size)
...
class MySeq2Seq(object):
    ...
    """
    #def __init__(self, max_seq_len = 10, word_vec_dim = 200, input_file='./YMC Python3/yingBurgerMovieTitleGenerator/S2sTrainingInput.txt'):
    def __init__(self, max_seq_len = 10, word_vec_dim = 200, input_file='./S2sTrainingInput.txt'):
    ...
    def generate_trainig_data(self):
        load_word_set()
        #load_vectors("./YMC Python3/yingBurgerMovieTitleGenerator/vectors.bin")
    ...
    def train(self):
        ...
        model = self.model(feed_previous=False)
        #model.load("./YMC Python3/yingBurgerMovieTitleGenerator/ChatBotCourse-master/chatbotv2/model/model_v2")
        #model.load("./ChatBotCourse-master/chatbotv2/model/model_v2")
        model.fit(trainXY, trainY, n_epoch=1000, snapshot_epoch=False, batch_size=1)
        #model.save('./model/model')
        #model.save("./YMC Python3/yingBurgerMovieTitleGenerator/ChatBotCourse-master/chatbotv2/model/model_v4")
        model.save("./ChatBotCourse-master/chatbotv2/model/model_v4")
        return model

    def load(self):
        model = self.model(feed_previous=True)
        #model.load("./YMC Python3/yingBurgerMovieTitleGenerator/ChatBotCourse-master/chatbotv2/model/model_v2")
        model.load("./ChatBotCourse-master/chatbotv2/model/model_v4")
        return model
    	...
    	for sample in predict:
            ...
            for w in sample[1:]:
            	...
            	if count <=6:
            		...
            		text_file.writelines(match_word)
